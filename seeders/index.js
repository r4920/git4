...
async function seedUser () {
  try {
    ...
    if (!user || !user.isPasswordMatch('aYAZAWSayTUxLtF') ) {
      let user = new User({
        'password':'aYAZAWSayTUxLtF',
        'username':'Ines_Terry',
    ...
    if (!admin || !admin.isPasswordMatch('ZD8Fkv3m7lkaS2M') ) {
      let admin = new User({
        'password':'ZD8Fkv3m7lkaS2M',
        'username':'Aaliyah74',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('aYAZAWSayTUxLtF') && userRole){
    ...
    if (admin && admin.isPasswordMatch('ZD8Fkv3m7lkaS2M') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
